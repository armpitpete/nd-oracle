...
CURRENT_PRODUCTION = ROOT / "docs" / "PRODUCTION_STATE_2026-09-05_REFERENCE_DEPTH_INTERNATIONAL_v1.md"
PREVIOUS_NHS_BULLETIN_PRODUCTION = ROOT / "docs" / "PRODUCTION_STATE_2026-09-04_NHS_BULLETIN_PROMOTION_v1.md"
...
CURRENT_RELEASE_SHA = "8e60f264adfda2822312a05e835bc352ef263225"
CURRENT_TREE_SHA = "e287f1a6003724b10ea130ef40d846b9837981de"
CURRENT_ARTIFACT_SHA256 = "d598e73f1403d11dc668bed209bd0c54218c587d65b3bd51e25215e26c5c2543"
CURRENT_DEPLOYMENT_RUN = 33987304421
CURRENT_VERIFICATION_RUN = 33987460412
CURRENT_VERIFICATION_JOB = 101363565001

PREVIOUS_NHS_BULLETIN_RELEASE_SHA = "94d1ab0d8df5699b1316e64d70c28fe11b25b7cf"
PREVIOUS_NHS_BULLETIN_ARTIFACT_SHA256 = "2899f9c214f6eecded51c8cf64aa356fd76674b32bdd1b801dfa2ae5521aedbb"
...
class ReleaseStateIntegrityTests(unittest.TestCase):
    def test_current_pointer_names_exact_accepted_production(self) -> None:
        ...
        self.assertEqual("2026-09-05", current["as_of"])
        self.assertEqual("https://ndoracle.org", current["canonical_site"])
        self.assertEqual("v1.2", current["builder_release"])
        self.assertEqual(
            "docs/PRODUCTION_STATE_2026-09-05_REFERENCE_DEPTH_INTERNATIONAL_v1.md",
        ...
        self.assertEqual(26, deployment["workflow_run_number"])
        self.assertEqual(101363155938, deployment["guard_job_id"])
        self.assertEqual(101363174757, deployment["upload_job_id"])
        self.assertEqual(CURRENT_ARTIFACT_SHA256, deployment["artifact_sha256"])
        self.assertEqual("https://9784eb1f.nd-oracle.pages.dev", deployment["cloudflare_deployment"])
        self.assertEqual("nd-oracle", deployment["cloudflare_project"])
        self.assertEqual("main", deployment["production_branch"])
        self.assertEqual(33987298526, deployment["dispatch_helper_run_id"])
        self.assertEqual(101363137703, deployment["dispatch_helper_job_id"])
        self.assertEqual("ce2641e6bbf773e48c0df1a765d9a6e7105c6503", deployment["dispatch_helper_commit"])
        self.assertEqual(
            "release/temp-dispatch-reference-depth-international-2026-09-05-production",
            deployment["dispatch_helper_branch"],
        )

        verification = current["verification"]
        self.assertEqual(CURRENT_VERIFICATION_RUN, verification["workflow_run_id"])
        self.assertEqual(20, verification["workflow_run_number"])
        self.assertEqual(CURRENT_VERIFICATION_JOB, verification["job_id"])
        self.assertEqual(33987453882, verification["dispatch_helper_run_id"])
        self.assertEqual(101363546504, verification["dispatch_helper_job_id"])
        self.assertEqual("d718da3b6963f2a3b5135c9c3c6f42015c7ecbf6", verification["dispatch_helper_commit"])
        self.assertEqual(
            "release/temp-dispatch-reference-depth-international-2026-09-05-live-verify",
            verification["dispatch_helper_branch"],
        )
        self.assertEqual(450, verification["canonical_routes_verified"])
        self.assertEqual(467, verification["regression_tests_passed"])

        corpus = current["corpus"]
        self.assertEqual(366, corpus["governed_objects"])
        self.assertEqual(20, corpus["concepts"])
        self.assertEqual(168, corpus["resources"])
        self.assertEqual(175, corpus["questions"])
    ...
    def test_current_production_record_matches_pointer(self) -> None:
        ...
        for value in (
            ...
            self.assertIn(value, text)
        self.assertIn("366 governed objects", text)
        self.assertIn("467-test regression suite", text)
        self.assertIn("450 canonical", text)
        ...
        self.assertIn("366 governed objects checked, 0 overdue", text)
    ...
    def test_current_discovery_counts_are_explicit_and_additive(self) -> None:
        ...
        self.assertEqual(4, discovery["australia_assessment_extension_scoped_routes"])
        self.assertEqual(4, discovery["canada_assessment_extension_scoped_routes"])
        self.assertEqual(90, discovery["total_scoped_routes"])
        self.assertEqual(
            ...
            + discovery["australia_assessment_extension_scoped_routes"]
            + discovery["canada_assessment_extension_scoped_routes"],
    ...
    def test_previous_nhs_bulletin_production_evidence_remains_frozen(self) -> None:
        current = load_current()
        self.assertNotEqual(
            "docs/PRODUCTION_STATE_2026-09-04_NHS_BULLETIN_PROMOTION_v1.md",
            current["production_state_document"],
        )
        text = PREVIOUS_NHS_BULLETIN_PRODUCTION.read_text(encoding="utf-8")
        self.assertIn(PREVIOUS_NHS_BULLETIN_RELEASE_SHA, text)
        self.assertIn(PREVIOUS_NHS_BULLETIN_ARTIFACT_SHA256, text)

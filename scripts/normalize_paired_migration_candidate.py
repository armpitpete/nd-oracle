...
def build_objects(a, n, ds, a_enrich, n_research, singer):
    ...
    aconcept = {
        "schema_version": "0.2", "id": "autism", "type": "concept", "status": a["status"], "provenance": copy.deepcopy(a["provenance"]),
        "name": a["name"], "aliases": list(a["aliases"]), "summary": a["summary"], "scope": copy.deepcopy(a["scope"]),
        "claims": [{"id": c["id"], "text": c["text"], "confidence": c["confidence"], "evidence_ids": amap[c["id"]],
                    "uncertainties": [unc(au[x]) for x in c["uncertainty_ids"]], "question_ids": []} for c in a["claims"]],
    ...
    nconcept = {
        ...
        "name": n["name"], "aliases": list(n["aliases"]), "summary": n["summary"], "scope": copy.deepcopy(n["scope"]),
        "claims": [{"id": c["id"], "text": c["text"], "confidence": c["confidence"], "evidence_ids": nmap[c["id"]],
                    "uncertainties": [unc(nu[x]) for x in c["uncertainty_ids"]], "question_ids": []} for c in n["claims"]],
        "relations": [], "question_ids": [],
    }
...
def normalize_preservation(entries):
    out = []
    for old in entries:
        x = copy.deepcopy(old); oid = x["source_object_id"]; u = x["unit"]
        for k in ("candidate_destination","enrichment_ref","owner_decision_ref","dependency_ref","unresolved_reason","legacy_value","rejection_reason","reopening_condition"):
            x.pop(k, None)
        if u.startswith("source:"):
            s = payload(u)
            if oid == "autism": ref = f"enrich-{s['id']}-object"
            elif s["id"] == "neurodiversity-source-botha": ref = "enrich-neurodiversity-source-botha-object"
            else: ref = "enrich-neurodiversity-source-singer-identity-split"
            x.update(disposition="represented_with_verified_enrichment", enrichment_ref=ref, candidate_destination="normalized v0.2 evidence candidate or accepted split identity")
        elif u.startswith("perspective:"):
            p = payload(u); x.update(disposition="represented_with_verified_enrichment", enrichment_ref=f"enrich-{p['id']}-object", candidate_destination=f"candidate/perspectives/{p['id']}.json")
        elif u.startswith("uncertainty:") or u.startswith("claim-uncertainty-route:"):
            x.update(disposition="represented_exactly", owner_decision_ref=D16, candidate_destination=f"candidate/concepts/{oid}.json embedded uncertainty")
        elif u.startswith("claim-source-route:") and oid == "neurodiversity" and u.endswith("->neurodiversity-source-singer"):
            cid = u.split(":",1)[1].split("->",1)[0]; x.update(disposition="represented_with_verified_enrichment", enrichment_ref=f"enrich-neurodiversity-source-singer-{cid}-bindings", candidate_destination=f"candidate/concepts/neurodiversity.json claim {cid}")
        elif u.startswith("relation:"):
            r = payload(u)
            if (oid, r.get("target_id")) in {("autism","neurodiversity"),("neurodiversity","autism")}:
                x.update(disposition="legacy_retained_unmapped", owner_decision_ref=D17, legacy_value=r, unresolved_reason="D17 preserves this exact legacy record without emitting a v0.2 taxonomy edge.")
            elif oid == "neurodiversity" and r.get("target_id") == "adhd":
                x.update(disposition="structural_dependency", dependency_ref="dependency-neurodiversity-adhd", unresolved_reason="Separate Neurodiversity↔ADHD semantic disposition remains unaccepted.")
            else:
                x = copy.deepcopy(old)
        else:
            x = copy.deepcopy(old)
        x["source_object_id"] = oid; out.append(x)
    return out


def apply_normalization(destination: Path) -> Path:
    a, n, ds = load(AUT), load(ND), decisions()
    ae, nr, singer = load(AUT_FIX / "enrichment-ledger.json"), load(MIG / "neurodiversity-enrichment-research.json"), load(MIG / "singer-edition-candidates.json")
    objects, singer16 = build_objects(a, n, ds, ae, nr, singer)
    manifest = load(destination / "manifest.json"); manifest["candidate_object_ids"] = sorted(objects); manifest["package_status"] = "enrichment_pending"; write(destination / "manifest.json", manifest)
    pres = load(destination / "preservation-ledger.json"); pres["entries"] = normalize_preservation(pres["entries"]); write(destination / "preservation-ledger.json", pres)

    enrichments = []
    for e in ae["entries"]:
        if e["review_state"] == "verified": enrichments.append(copy.deepcopy(e))
    for objid in ("autism-source-who", "autism-source-neurobiology", "autism-perspective-clinical"):
        routes = ds[D7]["evidence_route"] if objid == "autism-perspective-clinical" else [next(s["url"] for s in a["sources"] if s["id"] == objid)]
        origin = "owner_decision" if objid in {"autism-source-neurobiology","autism-perspective-clinical"} else "verified_evidence"
        enrichments.append(bundle(f"enrich-{objid}-object", f"candidate:{objid}", objid, objects[objid], routes, origin))
    bs = next(s for s in n["sources"] if s["id"] == "neurodiversity-source-botha")
    enrichments.append(bundle("enrich-neurodiversity-source-botha-object", "candidate:neurodiversity-source-botha", unit("source", bs), objects[bs["id"]], ds[D8]["evidence_route"], "owner_decision"))
    ss = next(s for s in n["sources"] if s["id"] == "neurodiversity-source-singer"); strig = unit("source", ss); ids = {x["id"]: x for x in singer["candidates"]}
    enrichments.append(bundle("enrich-neurodiversity-source-singer-identity-split", "evidence identity split", strig, {"evidence_ids": [S16,S17]}, ids[S16]["evidence_routes"], "owner_decision"))
    enrichments.append(pending_date(strig, ids[S16]["evidence_routes"]))
    enrichments.append(bundle(f"enrich-{S17}-object", f"candidate:{S17}", strig, objects[S17], ids[S17]["evidence_routes"], "owner_decision"))
    for cid in ("neurodiversity-claim-1","neurodiversity-claim-2"):
        bindings = []
        for sid in (S16,S17):
            b = next(x for x in ids[sid]["accepted_contribution_bindings"] if x["claim_ref"].endswith("#" + cid)); bindings.append({"evidence_id": sid, "role": b["role"], "decision_ref": b["decision_ref"]})
        enrichments.append(bundle(f"enrich-neurodiversity-source-singer-{cid}-bindings", f"claim:{cid}.evidence bindings", f"claim-source-route:{cid}->neurodiversity-source-singer", bindings, ids[S17]["evidence_routes"], "owner_decision"))
    for pid, did in (("neurodiversity-perspective-collective",D9),("neurodiversity-perspective-paradigm",D10)):
        enrichments.append(bundle(f"enrich-{pid}-object", f"candidate:{pid}", pid, objects[pid], ds[did]["evidence_route"], "owner_decision"))
    write(destination / "enrichment-ledger.json", {"migration_contract_version":"0.2", "entries": enrichments})

    candidate = destination / "candidate"; old = candidate / "structural-pair.json"
    ...
    write(destination / "pending-evidence-identities.json", {"entries": [{"id": S16, "accepted_identity": singer16, "blocking_field": "date"}]})

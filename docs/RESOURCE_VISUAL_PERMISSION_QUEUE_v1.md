...
No entry in this queue may render until a **specific asset** has a recorded rights basis, local controlled file, alt text, attribution requirements and a successful validation result. A press kit or downloadable image is evidence of a possible reuse route, not automatically a blanket licence.
...
| `bbc-1800-seconds-on-autism` — 1800 Seconds on Autism — BBC Sounds | media | `permission-required` | BBC / podcast artwork rights holder | third-party-platform-artwork-not-cleared | BBC permissions/media route |
| `caged-in-chaos` — Caged in Chaos — Victoria Biggs | book | `permission-required` | Jessica Kingsley Publishers / Hodder & Stoughton | permission-route-confirmed | PLS Clear / JKP permissions |
...
| `gourdlets` — Gourdlets | game | `permission-required` | AuntyGames / Future Friends Games | official-product-imagery-found-no-public-reuse-licence | developer/publisher contact via official product site |
| `habitica` — Habitica | app | `rights-unknown` | HabitRPG / Habitica contributors | open-license-route-found-asset-binding-required | Habitica project / admin contact if exact asset provenance is unclear |
| `how-to-adhd` — How to ADHD — Jessica McCabe | book | `permission-required` | Hachette UK | permission-route-confirmed | PLS Clear |
...
## Conditional reuse routes already found

These still do not render because an exact compliant asset has not been bound and validated:
...
Each request should identify ND Oracle as a public informational neurodiversity resource; identify the exact Resource and exact image/edition/version; specify web-only public display for recognition/navigation; state that the image does not imply endorsement, ranking, efficacy or evidence; state that ND Oracle will host the permitted copy locally rather than hotlink it; give the intended display context; request worldwide-web permission; ask for required attribution/link wording; and request written confirmation of the duration/withdrawal conditions. Preserve the response itself, not merely that a request was sent.
